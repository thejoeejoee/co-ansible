...
- **Casdoor's `app-built-in` client secret is publicly readable** via unauthenticated `GET /api/get-application?id=admin/app-built-in` (Casdoor's own design — the app exists so anyone can look up its OIDC metadata). `roles/auth/tasks/bootstrap.yml` deliberately authenticates with a `built-in/admin` session cookie instead, never with that client secret.
- **Casdoor's default `built-in/admin` password is `123`** on first boot of an empty SQLite volume. `bootstrap.yml` rotates it to `auth__admin_pass` via `/api/set-password` in the same run — don't skip that step if hand-rolling a Casdoor deploy outside this role.
